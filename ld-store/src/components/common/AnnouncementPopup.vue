...
function dismiss(mode = 'session') {
  if (!active.value) return
  trackAnnouncement(active.value, 'close', route.meta.layout === 'seller' ? 'seller' : 'storefront')
...
watch(active, async (item, old) => {
  await nextTick()
  if (item && active.value && !dialog.value.open) {
