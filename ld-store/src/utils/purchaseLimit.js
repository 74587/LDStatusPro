...
export function formatPurchaseLimitLabel(limit, { loggedIn = false } = {}) {
  ...
  if (!loggedIn || !Number.isFinite(limit.remainingQuantity)) {
    return `每位用户累计限购 ${limit.quantity} 件${loggedIn ? '' : '，登录后查看余量'}`
  }
  if (isPurchaseLimitReached(limit)) {
    return `已达累计限购（${limit.quantity}/${limit.quantity}）`
  }
  return `累计限购 ${limit.quantity} 件 · 还可兑换 ${limit.remainingQuantity} 件`
}
