...
async function loadPriceInfo() {
  ...
  try {
    const result = await api.get('/api/image/price-info', { auth: 'required' })
...
async function startUpload() {
  ...
  if (isFreeUser.value) {
    try {
      const result = await api.get('/api/image/free-credential', { auth: 'required' })
  ...
  try {
    const result = await api.post('/api/image/create-order', undefined, { auth: 'required' })
...
async function checkPayment() {
  ...
  try {
    const result = await api.get(`/api/image/check-payment?orderNo=${paymentOrderNo.value}`, { auth: 'required' })
...
async function doUpload() {
  ...
  try {
    ...
    const result = await api.upload('/api/image/upload', formData, { auth: 'required' })
...
async function doDelete() {
  ...
  deleting.value = true
  try {
    const result = await api.delete(`/api/image/${deleteTarget.value.id}`, { auth: 'required' })
    if (result.success) {
      toast.success('图片已删除')
      // 从历史记录中移除
...
async function loadHistory() {
  ...
  try {
    const result = await api.get('/api/image/history', { auth: 'required' })
