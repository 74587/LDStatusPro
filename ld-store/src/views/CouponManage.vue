  ...
  <div class="manage-page">
    <main class="manage-shell">
      ...
      <section v-else>
        ...
        <nav v-if="!loading && !loadError && pagination.totalPages > 1" class="pagination" aria-label="已发放优惠券分页">
          <button type="button" :disabled="pagination.page <= 1" @click="goCampaignPage(pagination.page - 1)">上一页</button>
          <span>第 {{ pagination.page }} / {{ pagination.totalPages }} 页，共 {{ pagination.total }} 个活动</span>
          <button type="button" :disabled="pagination.page >= pagination.totalPages" @click="goCampaignPage(pagination.page + 1)">下一页</button>
        </nav>
      </section>
    </main>
  </div>
...
async function loadCampaigns(page = pagination.value.page || 1) {
  loading.value = true; loadError.value = ''
  const result = await fetchSellerCouponsRequest({ page, pageSize: pagination.value.pageSize })
  if (result.success) {
    campaigns.value = result.data?.items || []
    pagination.value = result.data?.pagination || { page, pageSize: 20, total: campaigns.value.length, totalPages: campaigns.value.length ? 1 : 0 }
  }
  else loadError.value = result.error || '优惠券列表加载失败'
  loading.value = false
}
function goCampaignPage(page) {
  if (loading.value || page < 1 || page > pagination.value.totalPages) return
