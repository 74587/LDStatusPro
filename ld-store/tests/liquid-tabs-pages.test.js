...
describe('migrated seller pages', () => {
  ...
  it('opens other categories without changing the current filter and supports Escape', async () => {
    const { wrapper, router } = await page(Orders, '/seller/orders?source=product&status=paid', { sellerMode: true })
    vi.useFakeTimers({ toFake: ['setTimeout', 'clearTimeout'] })
    const trigger = wrapper.get('.other-trigger')
    trigger.element.focus()
    await trigger.trigger('click')
    expect(router.currentRoute.value.query.status).toBe('paid')
    expect(wrapper.find('.other-menu').exists()).toBe(true)
    expect(wrapper.get('.other-menu').text()).not.toContain('上传失败')
    await trigger.trigger('keydown', { key: 'Escape' })
    expect(wrapper.find('.other-menu').exists()).toBe(false)
    expect(document.activeElement).toBe(trigger.element)
    await trigger.trigger('click')
    await buttonByText(wrapper.get('.other-menu'), '已完成').trigger('click')
    await vi.advanceTimersByTimeAsync(150)
    await flushPromises()
    expect(router.currentRoute.value.query.status).toBe('completed')
    expect(wrapper.get('.other-trigger').classes()).toContain('active')
    expect(wrapper.get('.selected-category').text()).toBe('已完成')
  })
  ...
  it('retains debounced order switching, page reset, and the other-status API mapping', async () => {
    ...
    await vi.advanceTimersByTimeAsync(150)
    await flushPromises()
    expect(router.currentRoute.value.query.status).toBe('refund')
    ...
    await flushPromises()
    expect(wrapper.get('.other-trigger').classes()).toContain('active')
    expect(wrapper.get('.selected-category').text()).toBe('已发货')
  })
})
