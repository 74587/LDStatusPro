...
| 未注册路由或已确认不可见内容 | 页面未找到，不显示原内容 | 默认品牌图 | `24 / 7` | HTTP 404 |
...
api2 明确返回 404 时，Worker 输出 404 分享页；api2 超时、5xx 或返回无效结构时，Worker仍返回对应页面的通用卡片和 HTTP 200，避免瞬时故障被解释为内容下架。
...
`revision` 根据标题、摘要、核心数字、内容图和更新时间等字段生成。内容变更后图片 URL 自动变化，不需要主动清 Cloudflare Cache；Worker 只把通过 PNG 签名、1 MiB 上限和 1200×630 尺寸检查的成功响应写入 Cache API。
